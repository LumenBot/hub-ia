...
**Dernière mise à jour :** 26 mai 2026 (S2.9 Lot Dev `--filter-unit` livré côté Plateforme)
**Sprint en cours :** **S2.9 — Lot Dev `--filter-unit` livré**. Sur BRIEF-CC-S2.9 (PR #107 mergée, commit `c3eefb3`) + SPEC v2.3 en vigueur. **3ᵉ sprint consécutif avec Lot Dev Plateforme dès démarrage** (codifié SPEC v2.3 §allocation D-030). Livré ce jour : **bug-fix flag `--filter-unit {standard,adversarial,all}` dans `rag/code/eval/run_eval.py`** (jamais implémenté côté code malgré spec dans briefs S2.7/S2.8, 2 occurrences contournées Desktop-side par 2 sous-YAML dérivés). Correctif minimal : constante `FILTER_UNIT_CHOICES` + fonction `filter_questions_by_unit` (union de 2 signaux pour détection adv — `unit:` explicite OU `is_adversarial()` sémantique) + flag argparse `choices=` (double validation) + `ValueError` fail-fast sur valeur inconnue + warning stderr sur filtre vide. **DIAGNOSTIC-FILTER-UNIT-S2.9.md** (état + historique + cause root + garde-fou proposé SPEC v2.4) + **VALIDATION-SCORING-S2.9-FILTER-UNIT.md** (5 cas obligatoires SPEC v2.2/v2.3 § Validation manuelle, 5/5 conforme). **9 tests pytest verts** → **243/243** total (+9 vs S2.8, 0 régression). Coût Anthropic Lot Dev = **0 $**. PR à ouvrir depuis `claude/execute-s29-lot-dev-filter-unit`. **Reste à faire S2.9 :** Cowork (Lot F.9 patches R11 éditoriaux ~85 manquements) → Desktop (Lot I référence post-patches ~0,50 $ + Lots Bench-1 à Bench-5 reranking/top-k 3/embeddings-large/BM25+dense/Haiku 4.5 sur eval ~3,00 $) → Plateforme (Lot J RAPPORT-CC-S2.9 + analyse comparative 5 benchmarks + arbitrage final SPEC v2.4 + PR finale).

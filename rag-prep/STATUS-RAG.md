...
**Dernière mise à jour :** 23 mai 2026 (S2.7 Lot Dev livré — extension run_eval mode adversarial, 204/204 tests)
**Sprint en cours :** **S2.7 ouvert (D-030 enrichie — Plateforme dès le démarrage)**. Lot A (SPEC v2.1 + BRIEF-CC-S2.7, PR #96) mergé. **Lot Dev ✅ livré** (Plateforme, branche `claude/execute-s27-lot-dev-adversarial`) : extension `run_eval.py` mode adversarial (`is_adversarial` + 7 marqueurs refus `REFUSAL_MARKERS` + `evaluate_adversarial` 3 verdicts refus_correct/refus_partiel/hallucination + reporting 2 blocs), **+14 tests (204/204 verts)**, README mis à jour, 0 $. Reste : Lot B (sondage vague 7), Lot F.7 (5 fiches outils + golden set adversarial), Lot I (Desktop eval standard + adversarial), Lot J (RAPPORT-CC-S2.7 focus overfit vs robustesse + PR finale).

...
**Dernière mise à jour :** 26 mai 2026 (S2.8 Lot Dev R11 livré côté Plateforme — citation_audit.py + tests + VALIDATION-SCORING)
**Sprint en cours :** **S2.8 — Lot Dev R11 livré côté Plateforme**. Sur BRIEF-CC-S2.8 (branche `s2.8-lot-a-spec-v22-brief` non mergée, commit `c6a31ef` Cowork-side) + SPEC v2.2 en vigueur. Livré ce jour : **`rag/code/audit/citation_audit.py`** (extension audit pipeline, règle R11 « outil glossarié → wikilink à la 1re occurrence ») + **28 tests pytest verts** (121/121 sur tout `rag/code/audit/`, non-régression audit-md-rag intacte) + **VALIDATION-SCORING-S2.8-R11.md** (5 cas SPEC v2.2 obligatoire — 3 positifs + 2 négatifs, tous conformes 5/5) + **rapport réel `audit-md-rag-R11-s2.8.md` + JSON** sur le vault actuel : **6 fiches outils parsées, 36 outils canoniques, 85 manquements détectés sur 21/32 fichiers** (3 mentions correctement wikilinkées). Dérive R11 attendue (la règle n'était pas implémentée avant aujourd'hui) → patch éditorial Cowork ~30-60 min groupé, ne bloque pas Lot I. Coût Anthropic Lot Dev = **0 $** (dev sans appel API). Tests + README MAJ + JOURNAL + STATUS MAJ. PR à ouvrir depuis `claude/execute-pilot-batches-mBSIp`. **Reste à faire S2.8 :** Cowork (sondage D-026 vague 8 + production 4 architectures A1-A4 + golden set +6-8 std +2 adv) → Desktop (Lot I eval extended + alerte vault > 400 chunks) → Plateforme (Lot J RAPPORT-CC-S2.8 + PR finale).

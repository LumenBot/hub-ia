...
**Dernière mise à jour :** 27 mai 2026 (S2.9 Lot J livré — RAPPORT-CC-S2.9 + arbitrage final, sprint clôturé côté Plateforme)
**Sprint S2.9 clôturé côté Plateforme** — tous les lots livrés : A (SPEC v2.3 + BRIEF, PR #107), Lot Dev `--filter-unit` (PR #108), F.9 patches R11 vague 5-7 (PR #109), Lot I baseline + Bench-2/3/4/5 + comparatif (PR #110), F.9-bis patches R11 résiduels vague 8 (PR #111), Lot J (rapport + PR finale, en cours). **Finding central : 2 optimisations adoptées en prod — BM25+dense hybrid (résout AP-8 q-083 : 20/20 sans modif éditoriale) + Haiku 4.5 par défaut (qualité 19/20 préservée, -76 % coût / -58 % p50)**. Pattern « sprint tech debt pur » inédit validé empiriquement (4 PRs, ~1,65 $ Anthropic — inverse tendance 2,5-2,85 $/sprint). R11 audit clean (0/70). 3ᵉ sprint consécutif Lot Dev Plateforme dès démarrage (SPEC v2.3 §D-030 confirmé empiriquement). 4 propositions SPEC v2.4 : pattern curatif AP-8 BM25 hybrid, Haiku baseline prod + routing Sonnet, anti-pattern 3ᵉ occurrence brief CLI, sous-set 15-25q par défaut benchmarks. Pistes S2.10 prioritaires : combinaison Haiku + BM25 hybrid (projection 20/20 + p50 ~6 s + 0,12 $/20q) + intégrations prod Lot Dev Plateforme + bench-1 reranking si capacité acquise + démarrage vague 9. Suite tests 243/243. RAPPORT-CC-S2.9.md produit (8 sections, ~3500 mots). PR finale à ouvrir depuis `claude/execute-s29-lot-j-rapport`. En attente merge Blaise + arbitrage Cowork des 4 propositions SPEC v2.4 + arbitrage Blaise voie reranking.
**Sprint en cours :** **S2.9 — Lot I + Benchmarks livré (Desktop)**. Sur SPEC v2.3 + BRIEF-CC-S2.9. **Lot A (PR #107), Lot Dev `--filter-unit` (PR #108), Lot F.9 patches R11 (PR #109) mergés sur main `43173f1`**. **Lot I + 4 benchs ✅** (Desktop, branche `s2.9-eval-reference`) sur sous-set 20q (14 adv + 6 std clés ; brief 118q incompatible avec cap 3,50 $ → arbitrage user pour sous-set). **Résultats clés** : (a) **baseline** Sonnet/k=5/embed-small = 19/20 (q-083 KO, 14/14 adv), p50 12s/p90 20s, 0,41 $ ; (b) **bench-4 BM25+dense hybrid (RRF) = 20/20 🎯** seul bench qui résout q-083 (outils-llm cité grâce au signal lexical), coût ≈ baseline, p90 +10 % — **à adopter en prod retrieval** ; (c) **bench-5 Haiku 4.5 = 19/20 (qualité préservée)**, p50 5s (−58 %), coût 0,095 $ (−76 %) — **pivot économique majeur**, à adopter par défaut avec routing Sonnet ad-hoc ; (d) **bench-3 embed -large** régresse adversarial 14→12, pas de gain — skipper ; (e) **bench-2 top_k=3** régression std (q-030 perd pr-07), latence −16 %, trade-off mitigé — skipper. **Bench-1 reranking non exécuté** : capacités absentes (clés Cohere/Voyage ; sentence_transformers non installé, lourd ~500 MB). À reprogrammer S2.10. **R11 résiduels** : 27 manquements sur 6 fichiers vague 8 (architectures + pattern-grille) vs 0 attendu par brief → patch éditorial complémentaire Cowork. **Coût total session** : 1,6452 $ Anthropic+OpenAI (bien sous cap 3,50 $ — économie 0,80 $ liée à bench-1 non exécuté). **Discipline SPEC v2.0 hygiène merge OK** (aucun marqueur). **Reco SPEC v2.4 / S2.10** : adopter BM25 hybrid + Haiku par défaut ; benchmarker la combinaison (S2.10) ; patcher R11 résiduel. Wrappers `rag/eval/bench_runner.py` (top_k/embed/gen/collection) + `bench_runner_hybrid.py` (BM25+RRF) créés, `requirements.txt` enrichi de `rank_bm25`. Reste : Lot J RAPPORT-CC-S2.9 + arbitrage SPEC v2.4 + intégration BM25 hybrid dans code prod (S2.10).
**Sprint S2.8 clôturé côté Plateforme** — tous les lots livrés : A (SPEC v2.2 + BRIEF, PR #102), F.8+G+H (5 archis A1-A4-Hybride + brique pattern-grille + golden set 118q, PR #103), Lot Dev R11 (citation_audit.py + 28 tests + VALIDATION-SCORING 5/5 + 85 manquements détectés, PR #104), Lot I (eval 118q standard 103/104 + adversarial 14/14, PR #105), Lot J (rapport + PR finale, en cours). **Robustesse RAG confirmée sur panel élargi : 103/104 std (99 %) + 14/14 adv (100 %) sur 118 questions**, latence p50 19/p90 22 s standard sous cibles, coût 2,8466 $. **🟡 Alerte vault > 400 atteinte** (444 chunks) → **recommandation explicite S2.9 sprint dédié optimisation** (reranking, top-k 5→3, embeddings -large, BM25+dense, Haiku 4.5 sur eval, --filter-unit fix). q-083 cas-école nouveau pattern saturation cluster fiches sœurs (5 archis « Coût indicatif » étouffent outils-llm). Récurrence Lot Dev Plateforme dès démarrage validée 2ᵉ sprint (S2.7 mode adversarial + S2.8 R11) → proposition SPEC v2.3. Suite tests 234/234 (+28 citation_audit). RAPPORT-CC-S2.8.md produit (8 sections, ~3000 mots). PR finale à ouvrir depuis `claude/execute-s28-lot-j-rapport`. En attente merge Blaise + arbitrage Cowork 3 propositions SPEC v2.3 (cluster fiches sœurs → différenciation leads, officialiser Lot Dev Plateforme dès démarrage, fix --filter-unit).
**Sprint en cours :** **S2.8 — Lot I livré**. Lot A (SPEC v2.2, PR #102) + Lot F.8+G+H (5 archis A1-A4-Hybride + brique pattern-grille + golden set 118q = 104 std + 14 adv, PR #103) + Lot Dev R11 (citation_audit.py, `7fd78e2`) + PR #104 **mergés sur main** (`0c74352`). **Lot I ✅** (Desktop, branche `s2.8-eval-vague-8` depuis origin/main) : vault 38 MD / **444 chunks** (vague 8 +64). **Standard : 103/104 score global (99 %)** — cible ≥ 96/104 dépassée ; 1 seul échec q-083 (« Tarif API Claude Sonnet/Opus » — outils-llm rang #6 sim -0,067 hors top-5, étouffé par chiffres-macro + 3 chunks « Coût indicatif » des archis vague 8 ; non bloquant). **Non-régression intégrale** (9/9 questions surveillées : q-002/030/036/038/051/052/056/073/076 = 1). **Adversarial : 14/14 refus_correct, 0 hallucination, 0 partiel 🎉** — cible ≥ 12/14 dépassée. **PR #100 (S2.7 Dev fix harness adversarial) validée empiriquement** — 14/14 inclut les 12 questions S2.7 + 2 nouvelles q-adv-013/014. **Robustesse RAG confirmée excellente : 103/104 std + 14/14 adv**. Latence : standard p50 19,0/p90 22,0s (✅ p50≤22 p90≤26), adversarial p50 10,0/p90 12,0s (✅ p50≤12 p90≤17). Coût **2,8466 $** (cap 2,80-3,00 ✅). 🟡 **ALERTE VAULT > 400** confirmée (444 chunks) → **recommandation S2.9 sprint dédié optimisation** : reranking post-retrieval / réduction top_k / embeddings -large / index hybride BM25+dense / éviction low-signal. **Écart procédure récurrent (2ᵉ occurrence)** : flag `--filter-unit` toujours absent du code Lot Dev → contourné via 2 sous-golden-sets dérivés. À fixer durablement (Plateforme : ajout argparse). Discipline SPEC v2.0 hygiène merge OK. Pas de Lot Drer (1 échec isolé, non-régression intégrale). Reste : Lot J (RAPPORT-CC-S2.8 latence + S2.9 + q-083 + validation harness 14/14) + arbitrage Cowork pistes S2.9.
**Sprint S2.7 clôturé côté Plateforme** — tous les lots livrés (A PR #96, Dev PR #97, F.7+G+H PR #98, I PR #99, Dev fix PR #100 mergée, J = rapport + PR finale). Correctif harness adversarial v2 : **re-scoring 12/12** (vs 0/12 faux négatif), 206/206 tests, 0 $. **Robustesse RAG validée EXCELLENTE : 96/96 standard + 12/12 adversarial** → le score parfait S2.6 n'était pas de l'overfit. `rag-prep/reports/RAPPORT-CC-S2.7.md` produit (8 sections). PR finale ouverte depuis `claude/execute-s27-lot-j-rapport`. En attente merge Blaise + arbitrage Cowork 3 propositions SPEC v2.2 (validation manuelle nouvelle règle scoring, critère « citer pour expliquer le manque », extension golden set adversarial). Pistes S2.8 : R11 wikilinks (pré-requis 5+ fiches outils rempli), sprint latence vault > 400, vague 8 architectures.
**Sprint en cours :** **S2.7 — Lot I livré**. Lot A (SPEC v2.1, PR #96) + Lot Dev (mode adversarial run_eval, PR #97) + Lots F.7+G+H (5 fiches outils + brique souveraineté EU + golden set 108q = 96 std + 12 adv, PR #98) **mergés sur main** (`a7bb5c8`). **Lot I ✅** (Desktop, branche `s2.7-eval-vague-7-adversarial`) : vault 32 MD / **380 chunks** (vague 7 +62). **Standard : 96/96 sources + score global (100 %)** — cible ≥ 90/100 dépassée ; non-régression intégrale (q-002/q-030/q-036/q-038/q-051/q-052/q-056 = 1). **Adversarial : harness 0/12 refus_correct (11 halluc + 1 partiel) MAIS analyse = 12/12 refus textuels corrects → FAUX NÉGATIF du harness Lot Dev**, pas une faiblesse RAG. **2 bugs harness** : (1) `REFUSAL_MARKERS` ne contient PAS la phrase canonique du system prompt « je n'ai pas de réponse documentée » → 0/12 détectés ; (2) verdict `refus_correct` exige `not cited`, or 11/12 citent le contexte pour expliquer le manque. **→ Correctif harness requis (Plateforme) avant RAPPORT-CC-S2.7** : enrichir REFUSAL_MARKERS + faire primer le marqueur sur les citations + re-scorer (attendu ~12/12). Latence : standard p50 19,0s/p90 22,0s, adversarial p50 10,0s/p90 15,0s — **sous cibles p50≤21/p90≤25 ✅** (vault 380 chunks, bande SPEC v2.1 OK). Coût **2,5866 $** (≤ 3,00 ✅). Écart procédure résolu : flag `--filter-unit` inexistant → 2 sous-golden-sets dérivés. Pas de Lot Drer (aucune régression). Reste : correctif harness adversarial + Lot J RAPPORT-CC-S2.7.
**Sprint en cours :** **S2.6 — Lot I livré (eval 81/81 = 100 %)**. Lot A (SPEC v2.0 + brief, PR #89) + Lots F.6+G+H (vague 6 PR-09/PR-10/PR-11 + refonte vigilance-hallucinations + golden set 81q, PR #93) **mergés sur main** (`bd1ebc1`). **Lot I ✅** (Desktop, branche `s2.6-eval-vague-6` dérivée de main) : eval complète 81q sur vault 26 MD / **318 chunks** → **81/81 sources (100 %), 81/81 concepts ≥ 50 % (100 %), 81/81 score global (100 %)** — cibles ≥ 70/81 (86 %) et ≥ 73/81 (90 %) **largement dépassées**. **Non-régression S2.5 intégrale** (q-002/q-030/q-036/q-038/q-051/q-052/q-056 = 1). **Vague 6 (q-073→q-081) : 9/9** (8 cibles rang #1, q-076 #2 derrière vigilance-hallucinations). **Audit AP-7 PR-11 RÉUSSI** (module pivot absent du top-5 sur q-073 + q-076 hors scope). Ingestion new=30 (vague 6 + sections vigilance refonte) + updated=6 + deleted=1, total 289 → 318 chunks. **2 alertes non bloquantes** : (a) **coût Anthropic 2,0509 $ = cap 2,00 $ dépassé de +0,05 $** (81q × ~0,0253 $, cap calibré 75-80q → recalibrer ~2,10 $) ; (b) **latence p50 19,0s ✅ / p90 22,0s > borne 20s** (vault a franchi 300 chunks, hors bande SPEC v2.0 « 200-300 ») → **recommandation §Performances v2.1** pour S2.7 (étendre bande latence vault 300-400, ou reranking/top_k si dérive). Discipline SPEC v2.0 hygiène merge respectée (`git grep "<<<<<<<"` = vide, aucun marqueur). **Lot J ✅ livré** (RAPPORT-CC-S2.6 8 sections avec latence p50/p90 §3 + PR finale ouverte depuis `claude/execute-s26-lot-j-rapport`). **Sprint S2.6 clôturé côté Plateforme** — en attente merge Blaise + arbitrage Cowork 3 propositions SPEC v2.1 (recalibrage cap coût ~2,15 $, extension bande latence vault 300-400 chunks, pattern module pivot dense). Pas de Lot Drer (aucune régression).
...
| L2.6j | S2.6 Lot J — RAPPORT-CC-S2.6 + PR finale S2.6 | ✅ Fait (`rag-prep/reports/RAPPORT-CC-S2.6.md` 8 sections + PR finale ouverte depuis `claude/execute-s26-lot-j-rapport`) ; latence p50 19,0 s / p90 22,0 s inscrite §3 ; 3 propositions SPEC v2.1 à arbitrer Cowork (cap coût ~2,15 $, bande latence vault 300-400, pattern module pivot dense) ; 2 alertes non bloquantes documentées | Claude Code Plateforme |
| L2.7dev | S2.7 Lot Dev — extension `run_eval.py` mode adversarial (auto-dispatch + REFUSAL_MARKERS + 3 verdicts + reporting 2 blocs) | ✅ Fait (`6380d2d`, PR #97 mergée) ; 204/204 tests | Claude Code Hub IA Plateforme |
| L2.7f7gh | S2.7 Lots F.7 + G + H — 5 fiches outils (frameworks-rag/knowledge-mgmt/llm/observabilité-llm/workflow-automation) + brique souveraineté EU + golden set 108q (96 std + 12 adversariales) | ✅ Fait (`c7378b5`, PR #98 mergée) | Cowork Hub IA Plateforme |
| L2.7i | S2.7 Lot I — eval extended 108q (96 std + 12 adv) sur vault post-vague 7 (32 MD / 380 chunks) + latence p50/p90 par mode | ✅ Fait (`s2.7-eval-vague-7-adversarial`, dérivée de main `a7bb5c8`) : **Standard 96/96 (100 %)** — cible ≥ 90/100 dépassée, non-régression 7/7. **Adversarial : harness 0/12 refus_correct (11 halluc + 1 partiel) — MAIS analyse = 12/12 refus textuels corrects → FAUX NÉGATIF du harness Lot Dev** (REFUSAL_MARKERS sans la phrase canonique du system prompt « je n'ai pas de réponse documentée » + verdict exigeant `not cited`). Robustesse RAG réelle excellente. Latence standard p50 19,0/p90 22,0s, adversarial p50 10,0/p90 15,0s (✅ p50≤21/p90≤25). Coût **2,5866 $** (≤ 3,00 ✅). Ingestion new=62, total 318 → 380 chunks. **→ Correctif harness adversarial requis (Plateforme) avant RAPPORT-CC-S2.7**. Pas de Lot Drer (aucune régression) | Claude Code Desktop |
| L2.7devfix | S2.7 Lot Dev fix — correctif harness adversarial v2 (enrichissement REFUSAL_MARKERS + verdict assoupli) | ✅ Fait (PR #100 mergée) ; 206/206 tests ; re-scoring S2.7 12/12 (vs 0/12 faux négatif initial) | Claude Code Hub IA Plateforme |
| L2.7j | S2.7 Lot J — RAPPORT-CC-S2.7 + PR finale | ✅ Fait (PR #101 mergée `43d7154`) ; 8 sections ; 3 propositions SPEC v2.2 | Claude Code Hub IA Plateforme |
| L2.8a | S2.8 Lot A — SPEC v2.2 + BRIEF-CC-S2.8 sprint hybride R11 + vague 8 + 2 adv | ✅ Fait (`c6a31ef`, PR #102 mergée) | Cowork Hub IA Plateforme |
| L2.8f8gh | S2.8 Lots F.8 + G + H — 5 architectures A1-A4-Hybride + brique pattern-grille-decision-architecture + golden set 118q (104 std + 14 adv) | ✅ Fait (`40e1ad3`, PR #103 mergée) | Cowork Hub IA Plateforme |
| L2.8devr11 | S2.8 Lot Dev R11 — citation_audit.py (règle R11 wikilinks outils glossariés) + 28 tests + VALIDATION-SCORING + rapport vault actuel | ✅ Fait (`7fd78e2`, PR #104 mergée) ; 121/121 tests audit, 85 manquements R11 détectés (dérive attendue, patch éditorial Cowork groupé) | Claude Code Hub IA Plateforme |
| L2.8i | S2.8 Lot I — eval extended 118q (104 std + 14 adv) sur vault post-vague 8 (38 MD / 444 chunks) + latence p50/p90 par mode + alerte vault > 400 | ✅ Fait (`s2.8-eval-vague-8` depuis main `0c74352`) : **Standard 103/104 (99 %)** — cible ≥ 96/104 dépassée, 1 échec q-083 (« Tarif API Claude Sonnet/Opus » — outils-llm rang #6 sim -0,067 hors top-5, étouffé par chiffres-macro + 3 chunks « Coût indicatif » archis vague 8). **Adversarial 14/14 refus_correct (100 %) 🎉** — cible ≥ 12/14 dépassée, **PR #100 fix harness validée empiriquement** (incl. 2 nouvelles q-adv-013/014). Non-régression intégrale 9/9. Latence standard p50 19,0/p90 22,0s ✅ ; adversarial p50 10,0/p90 12,0s ✅ ; coût **2,8466 $** (cap 2,80-3,00 ✅). Ingestion new=64, total 380 → 444 chunks. **🟡 ALERTE VAULT > 400 CHUNKS → recommandation S2.9 sprint dédié optimisation** (reranking / top_k / embeddings -large / index hybride / éviction low-signal). **Écart procédure récurrent : `--filter-unit` toujours absent code Lot Dev** (2ᵉ occurrence) → contourné via 2 sous-golden-sets dérivés, à fixer durablement (Plateforme). Pas de Lot Drer (1 échec isolé, non bloquant) | Claude Code Desktop |

...
**Dernière mise à jour :** 13 mai 2026 (S2.3 Lot B livré — vague 3 produite : cu-026 + cu-027 + dep-08 + patch chiffres-macro I-D-005)
**Sprint en cours :** **S2.3 en cours** — Lots A + B + C livrés Cowork-side. Lot D en cours par Claude Code Plateforme (adapt `evaluate_one()` synonymes). En attente sync ascendante Blaise pour : (1) appliquer patch chiffres-macro-2026 v3.8.5 → v3.8.6, (2) ingest vault 10 → 13 fichiers, (3) débloquer Lot E Desktop (eval extended 42q post-Lot D mergé). Cible eval recalibrée : ≥ 35/42 sources (≥ 83 %).

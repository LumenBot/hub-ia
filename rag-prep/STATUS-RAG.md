...
**Dernière mise à jour :** 22 mai 2026 (S2.5 Lot Drer livré — rerun eval ciblé post-correctif retrieval)
**Sprint en cours :** **S2.5 en cours** — Lots A (SPEC v1.8) + S2.5.0 (3 patches leads) + E (draft sondage vague 5) mergés sur main (PR #74, `903e068`). **Lot Drer (Desktop) livré** : rerun eval ciblé 4 questions post-patches S2.5.0 → **3/4** (q-002 ✅ restauré rang #1, q-051 ✅ + q-052 ✅ sans régression, **q-030 ❌ patch lead pr-07 insuffisant** — top-10 saturé 100 % pr-08, pr-07 seulement rang #13). Coût 0,1022 $ (cible ≤ 0,15 $ ✅), latence ~17,8 s/q. **Signalement immédiat** : q-030 nécessite un **Lot S2.5.0-bis** (H2 réglementaire dédiée pr-07 / re-scope chunks non-lead pr-08 / lead vigilance-confidentialite — un patch lead seul ne suffit pas). Artefacts sur `claude/execute-s25-lot-drer-rerun-cibles`, PR à ouvrir. Rappel décision budgétaire (cumul S1→S2.5 Lot Drer ~5,20 $).
